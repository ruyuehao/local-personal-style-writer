...
`local-style-writer` 是一个完全运行在**端侧**、**零网络外联**的个性化写作 Skill。它用 QLoRA 从你的历史文章中学习个人笔风，再由本地 INT4 量化的 Qwen3-8B 模型直接生成「像你本人写的」原创内容——草稿全程不离开你的电脑。

与「把稿件丢给云端大模型」不同，本技能把**风格学习、推理生成、检索增强**三步全部留在本地完成，面向的是「既想要 AI 的效率，又不愿泄露未发表成果」的真实写作需求。一次微调、长期复用：你的风格画像会被持久化保存，换设备、重训都能继承。

核心能力包含三块：

- **个性化生成**：按主题 / 要点 / 目标字数 / 语气，结合你的风格画像产出原创正文（扩写 / 改写 / 续写）。
- **风格一致性分析**：对任意给定文本打分（0–100），量化「像不像本人写的」，可用于代笔鉴别、品牌语调巡检。
- **RAG 检索增强**：可选地参考你的历史文章片段，让新稿在主题与语气上更贴近过往爆款。

## 解决的问题

1. **风格缺失，千篇一律** — 通用大模型生成的文章缺少个人品牌语调，读起来「AI 味」重。本技能把你的笔风固化为可复用的风格画像，输出自带个人印记。
2. **隐私泄露风险** — 未发表的商业文案、内部周报、个人随笔一旦上传云端，便面临泄露与被用于训练的风险。本技能纯本地推理，草稿 0 网络外联。
3. **依赖网络与配额** — 云端 API 有延迟、限流、断网即不可用。本技能在断网 / 保密环境下照常工作，且无调用计费。
4. **好文风难沉淀** — 过往爆款文章的调性往往只存在于感觉里，难以提炼复用。RAG 增强让新稿自动靠拢你历史文章的表达习惯。
5. **「像不像我」难以量化** — 人工判断风格一致性主观且低效。风格分析模型给出可解释的指标（困惑度、句长方差、词汇匹配度），把主观感觉变成分数。

## 应用场景

- **自媒体 / 公众号运营** — 用你的固定调性批量产出同风格推文、随笔，保持账号人设不漂移。
- **企业品牌文案** — 产品推广、活动海报文案、对内周报，统一团队 / 个人的品牌语调。
- **专业与学术写作** — 在保留个人表达习惯的前提下，加速初稿与扩写，不被通用模板带偏。
- **代笔 / 版权鉴别** — 用风格一致性评分判断某段文本是否本人所写，辅助代笔识别、内容确权。
- **保密环境写作** — 无外网或合规要求严格的机构内，离线完成高质量写作辅助。
- **历史内容复用** — 开启 RAG，让新稿自动吸收你过往爆款文章的选题与语气，旧内容焕发新价值。
